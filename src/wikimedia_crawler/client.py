...
class WikimediaClient:
    ...
    ENDPOINTS = {
        ...
        'wikitravel': 'https://wikitravel.org/wiki/en/api.php'
    }

    def __init__(self, source):

        if source not in ('wikipedia', 'wikitravel'):
            raise NotImplementedError

        self.base_url = WikimediaClient.ENDPOINTS[source]
        self.config_file = 'logging.ini'
        self.logger = self._get_logger()
        self.logger.info('Crawler created')

        self.countries = self._read_countries()
        self.data_dir = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'data', source))
...
def main():
    ...
    crawler = WikimediaClient('wikipedia')
